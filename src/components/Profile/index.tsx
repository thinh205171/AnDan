        ...
        <div className='profile-panel'>
            <div className="profile-panel-container">
                ...
                <div className="profile-panel-container-detail">
                    ...
                    <img src={userInfo?.photo !== '' ? userInfo?.photo : "https://img.freepik.com/free-psd/3d-illustration-person-with-sunglasses_23-2149436188.jpg"} alt="avatar"
                    ...
                    <div className="profile-panel-container-detail-description">
                        ...
                        <div style={{ display: "flex", columnGap: "16px" }}>
                            <div className="profile-panel-container-detail-description-content">
                                <div className='profile-edit-btn' onClick={() => setOpenEdit(true)}><Edit style={{ color: "#E2BFB3" }} /> Chỉnh sửa thông tin</div>
                            </div>
                            <div className="profile-panel-container-detail-description-content">
                                <div className='profile-edit-btn' onClick={() => setOpenCode(true)}><Lock style={{ color: "#E2BFB3" }} /> Thay đổi mật khẩu</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <Dialog
                ...
                PaperProps={{
                    ...
                    onSubmit: async (event: React.FormEvent<HTMLFormElement>) => {
                        ...
                        try {
                            if (avatarUrl)
                                await apiupdateUser({ email: email, address: address, photo: avatarUrl, id: userInfo?.id, accountId: userInfo?.accountId }, userInfo?.id)
                            else
                                await apiupdateUser({ email: email, address: address, id: userInfo?.id, accountId: userInfo?.accountId }, userInfo?.id)
            ...
            >
                ...
                <DialogContent style={{ textAlign: "center" }}>
                    ...
                    <div style={{ marginTop: "12px", display: "flex", width: "50%", justifyContent: "space-between" }} >
                        <label htmlFor="avatar">Chọn ảnh đại diện</label>
                        <input type="file" id="Avatar" name="Avatar" accept="image/*" style={{ marginLeft: "8px" }} onChange={handleAvatarChange} />
                    </div>
                    <TextField
                        autoFocus
                        required
                    ...
                    onSubmit: async (event: React.FormEvent<HTMLFormElement>) => {
                        ...
                        try {
                            ...
                            if (password && cfPassword && curPass) {
                                const res = await apiCheckVerifyPassword(null, { username: user?.username.trim(), currentPassword: curPass, newPassword: password })
                                setOpenCode(false)
                            }
                        } catch (e) {
