...
import '../style.scss'
import { Article, Folder, Note, NotificationsActive, Square } from '@mui/icons-material'
        ...
        <Grid item xs={2} style={{ padding: 0 }}>
            <div className='sub-nav-bar sub-nav-bar-left'>
                ...
                <div className="sub-nav-bar-item">
                    ...
                    <div className="sub-nav-bar-item-content" style={{ paddingTop: "12px" }}>
                        <div className="sub-nav-bar-item-detail">
                            {
                                notis?.map((noti: any, index: any) => (
                                    <div key={index} style={{ display: 'flex', columnGap: "12px", cursor: "pointer" }}
                                        onClick={() => navigate(`/sub-menu-${noti?.docType}/detail-view/${noti?.docId}`)}
                                    >
                                        <NotificationsActive style={{ color: "rgb(239, 179, 142)" }} />{noti?.message}
                                    </div>
                                ))
                            }
                        </div>
